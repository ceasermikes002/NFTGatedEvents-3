...
contract EventOrganizer {
    ...
    mapping(address => mapping(uint256 => bool)) public userRegistrations; //To track if user has registered for an event to prvent double registration.
    ...
    function registerForEvent(uint256 eventId) external {
        ...
        require(hasRequiredNFT(nftRequired, msg.sender) >= 1, "NFTRequiredForEvent"); //Check if user has NFT.
